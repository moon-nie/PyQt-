...
# 결측 ML 대체(KNN/MICE)는 df_tool/ops_impute.py, 이상치 탐지는 df_tool/ops_outliers.py로
# 분리했다. 기존 `from df_tool.operations import ...` 경로 호환을 위해 파일 끝에서 re-export한다.
...
# --- leaf 도메인 re-export (호환 유지) ---------------------------------------
# 분리된 모듈의 함수를 operations 네임스페이스로 노출해 기존 import 경로를 유지한다.
# (ops_impute/ops_outliers는 호출 시점에만 operations 헬퍼를 참조하므로 순환 안전)
from df_tool.ops_impute import fill_na_knn, fill_na_mice  # noqa: E402
from df_tool.ops_outliers import (  # noqa: E402
    _outlier_mask_iqr,
    _outlier_mask_isolation_forest,

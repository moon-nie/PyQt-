...
### 크롤 취소·프리셋 · 분석 안정성·QA
- 크롤 **[작업 취소]**: poller + 브라우저 일괄 큐 + WebEngine fetch 중단 (`RenderedHtmlFetcher.cancel`)
- **규칙 프리셋** 저장/불러오기/삭제 (`~/.gridloom/crawl_presets.json`, `crawl_presets.py`)
- 분석 탭 matplotlib **지연 import** — 패키지 없어도 패널 모듈 로드 가능
- 단·이·다변량 **PNG 저장** 버튼
- `qa_analysis_panel_smoke` 이·다변량·이상치·KNN 적용 경로 보강
- `qa_crawl_smoke` 프리셋·cancel API 검증
...
### 분석 탭 ! 툴팁이 안 보이던 문제
- 전역 `QWidget` 스타일이 `QToolTip`을 깨뜨리던 문제 — `QToolTip` 스타일 명시
- `!` 호버/클릭 시 `QToolTip.showText`로 설명을 직접 표시
...
### 분석 탭 ! 도움말 UX 정리
- WhatsThis(?) 커서 제거 — 일반 툴팁으로 설명 표시
- 이상치 **방식 옆 ! 하나**만, 선택한 방식(IQR/Z/IF)에 맞게 설명 변경
- EDA 요약·HTML 리포트 등 버튼에서 ! 제거 — bins·top-N·이웃 수·IQR k 등 설정/용어만 유지
...
### 분석 탭 용어 도움말 · 신뢰도·유연성
- 설정 옆 **[!]** 툴팁: bins, top-N, IQR, Z-score, KNN/MICE, PCA 등 (`analysis_help` + `qt_help_tip`)
- KNN 미리보기 문구 정직화 (실제 impute 없이 “적용 시 채움” 안내)
- 열 미선택 시 KNN/MICE/이상치 **전체 숫자 열** 적용 전 확인
- 대용량에서 탭 전환 자동 그리기 생략 · 다변량 탭 열면 상관 행렬
- IQR 배수 k · MICE max_iter · Pair 최대 열(2–6) · 개요 표 최소/최대
- 개요 결측 차트가 단변량 축 설정을 더 이상 공유하지 않음
- 차트/bins/top-N 변경 시 단·이변량 자동 다시 그리기
...
### HTML EDA 리포트 풀 고도화
- 저장 기본명: `{연 파일명}_report.html` (원본 폴더 제안)
- 표지·KPI·규칙 기반 인사이트 · 전문 레이아웃 CSS
- 핵심 차트: 수치 분포 최대 6열 · 범주 빈도 최대 4열 · 결측·상관
- 섹션 추가: 범주형 요약 · IQR 이상치 · |r| 상위 상관
- `default_eda_report_filename` · `AnalysisPanel.get_source_path`

---

## v0.8.37 (2026-07-16)

### 값 중복 찾기: 여러 열 = 열마다 따로 (조합 키 아님)
- 아이디·이메일을 고르면 **각 열의 중복 유무**를 한 화면 요약 + 탭으로 표시
- `duplicate_value_reports` / 다이얼로그 `qt_duplicate_reports_dialog`
- 행 표시는 열별 중복의 합집합(또는 현재 탭만)

---

## v0.8.36 (2026-07-16)

### 행 정리 버튼 가시성 · 여러 열 값 중복 찾기
- 툴바 라벨 명확화: **값 중복 찾기** / **완전동일 행 제거** / **결측 행 제거** (+ 툴팁)
- [값 중복 찾기]에서 열 미선택 시 **여러 열을 한 번에** 선택 가능 (열별 검사는 v0.8.37)
- 완전동일 행·결측 행 제거 전 확인 대화상자 — 서로/채우기와 혼동 방지 문구
- 도움말·리포트 다이얼로그 문구를 동일 용어로 맞춤

---

## v0.8.35 (2026-07-16)

### 중복 찾기 버튼·중복 행만 표시 크래시 수정
- 툴바 [중복 찾기]: `clicked(bool)`을 열 이름으로 오인하던 문제 수정
- [중복 행만 표시]: 잘못된 위젯명(`search_column`)·페이지 라벨 갱신으로 앱이 죽던 문제 수정
- 필터 적용 전 선택 해제·다이얼로그 종료 후 지연 적용으로 안정화
...
### 중복 값 찾기 (고유값 부족 진단)
- 전체 행 3341 · 고유 3339처럼 **고유값이 모자랄 때** 어떤 값이 몇 번인지 바로 확인
- [중복 찾기] 버튼 · 열 헤더 우클릭 **중복 값 찾기…**
- 여러 열 선택 시 조합 키 기준 중복 검색
- [중복 행만 표시] → 표 필터 → [결과 추출]로 해당 행만 남기기 가능
- 로직: `operations.duplicate_value_report` / `duplicate_row_mask`

...
- **버전:** v0.8.42 (`df_tool/__init__.py`)
...
- **크롤링 탭** — 규칙 프리셋 · 작업 취소 · 로그인 브라우저 · JS 렌더 · 일괄 · 미리보기 리사이즈 · 열린 표에 키 병합
- **결측·이상치** — KNN/MICE 결측 대체, IQR·Z·Isolation Forest 이상치 제거
- **차트 꾸미기** — 색·글자·범례·색상맵 사용자 설정
- **결측치 채우기** (평균·중앙값·KNN·최빈값 등), **열 병합·열 분리**
- 검색 필터 ([검색] 버튼), **[결과 추출]** (필터된 행만 남기기), 찾기/바꾸기, 값 중복 찾기·완전동일 행 제거·결측 행 제거
- 왼쪽 위 코너 클릭·Ctrl+A 전체 선택, 창 위치·크기 기억 (`~/.gridloom/window.json`)
